...
{ head -n1 "$TMP/t6.tsv"; tail -n +2 "$TMP/t6.tsv" | sort -k1,1; } > "$OUT/task6.tsv"
...
{ head -n1 "$TMP/t8.tsv"; tail -n +2 "$TMP/t8.tsv" | sort -k3,3nr -k2,2; } > "$OUT/task8.tsv"
...
{ head -n1 "$TMP/t9.tsv"; tail -n +2 "$TMP/t9.tsv" | sort -k3,3nr -k2,2nr; } > "$OUT/task9.tsv"
...
{ head -n1 "$TMP/t10.tsv"; tail -n +2 "$TMP/t10.tsv" | sort -k3,3nr -k2,2nr; } > "$OUT/task10.tsv"
rm -rf "$TMP"
echo "Done"
